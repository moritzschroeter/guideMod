...
public class ollamaHandler {
    ...
    public void resetModel() {
        try {
            System.out.println("[GUIDE] Resetting model to: " + initLLM.getModelName());
            var llm = MuseumGuideClient.getLlmInstance();
            if (llm != null) {
                llm.reinitialize();
            } else {
                System.err.println("[GUIDE] LLM instance is null, impossible to reset.");
            }
        } catch (Exception e) {
            System.err.println("[GUIDE] Error resetting model: " + e.getMessage());
            e.printStackTrace();
        }
    }
}